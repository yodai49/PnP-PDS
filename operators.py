...
def get_operators(shape, gamma1, gamma2, lambda1, lambda2, phi, adj_phi, path_prox, x_0):
    def grad_f(x):
    ...
    def prox_h(x):
        ...
        #val  = x
        #epsilon = shape[0] * 0.1
        #if(np.linalg.norm(phi(x) - x_0) > epsilon):   
        #    val = x_0 + epsilon * (phi(x) - x_0) / np.linalg.norm(phi(x) - x_0)
        #return val
