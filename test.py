...
def eval_PSNR():
    path_obs = config['root_folder'] + 'Poisson_final/01.png_OBSRV_ours-C(blur)_gaussian-nl0.0(20231223-172705).png'
    path_true = config['root_folder'] + 'Poisson_final/01.png_TRUE_ours-C(blur)_gaussiannl0.0(20231223-172705).png'
    img_true = cv2.imread(path_true)
    img_obs = cv2.imread(path_obs)
    img_true = np.asarray(img_true, dtype="float32")/255.
    img_obs = np.asarray(img_obs, dtype="float32")/255.
    print(pds.psnr(img_true, img_obs))

def check_homogeneity(architecture = '', ch=3, epsilon = 0.05):
...
def eval_restoration(max_iter = 1000, gaussian_nl = 0.01, sp_nl = 0.01, poisson_noise = True, poisson_alpha = 1, gamma1 = 1.99, gamma2 = 1.99, alpha_n = 0.9, alpha_s = 0.9, myLambda=1, m1=25, m2=1, result_output = True, method = 'ours-A', architecture = '', r=0.5, deg_op = 'blur', ch = 3, gammaInADMMStep1=10):
    ...
    for path_img in path_images:
        ...
        if(deg_op == 'blur' or deg_op == 'Id'):
            ...
            print('ERROR: Gaussian noise cannot be added')
        ...
        if(deg_op == 'blur' or deg_op == 'Id'):
            ...
            print('ERROR: Sparse noise cannot be added')
        ...
        img_sol, s_sol, temp_c, temp_psnr, average_time = test_iter(x_0, img_obsrv, img_true, phi, adj_phi, gamma1, gamma2, alpha_s, alpha_n, myLambda, m1, m2, gammaInADMMStep1, gaussian_nl, sp_nl, poisson_alpha, path_prox, max_iter, method, ch, r)
...
#    psnr = eval_restoration(gaussian_nl=0.005, sp_nl=0.0, poisson_noise=False, poisson_alpha = 0, max_iter = 1200, gamma1 = 1, gamma2 = 0.99, r=1, alpha_n = 0.95, alpha_s = 0.95, myLambda=2, result_output=False, architecture='DnCNN_nobn_nch_3_nlev_0.01', deg_op = 'blur', method = 'comparisonA-1', ch = 3)
#    psnr = eval_restoration(gaussian_nl=0.01, sp_nl=0.0, poisson_noise=False, poisson_alpha = 0, max_iter = 1200, gamma1 = 1, gamma2 = 0.99, r=1, alpha_n = 0.95, alpha_s = 0.95, myLambda=2, result_output=False, architecture='DnCNN_nobn_nch_3_nlev_0.01', deg_op = 'blur', method = 'comparisonA-1', ch = 3)
#    psnr = eval_restoration(gaussian_nl=0.02, sp_nl=0.0, poisson_noise=False, poisson_alpha = 0, max_iter = 1200, gamma1 = 1, gamma2 = 0.99, r=1, alpha_n = 0.95, alpha_s = 0.95, myLambda=0.85, result_output=False, architecture='DnCNN_nobn_nch_3_nlev_0.01', deg_op = 'blur', method = 'comparisonA-1', ch = 3)
...
#    psnr = eval_restoration(gaussian_nl=0.01, sp_nl=0.0, poisson_noise=False, poisson_alpha = 0, max_iter = 1200, gamma1 = 0.1, gamma2 = 0.99, r=1, alpha_n = 0.95, alpha_s = 0.95, myLambda=0.1, result_output=False, architecture='DnCNN_nobn_nch_3_nlev_0.01', deg_op = 'blur', method = 'comparisonA-4', ch = 3)
